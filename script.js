...
function index(e){
  ...
  if(!e.connection.isLocal){ response.write('Sorry, this is a local application.'); response.close(); }

  if(request.queryItems['w']){
    output.widgetise(
      response,
      widgetise.create(
        appDir.resolve('template.zip'),
        JSON.parse(decodeURIComponent(request.queryItems['w'][0]))
      )
    );
  }
